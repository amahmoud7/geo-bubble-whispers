// SeatGeek API Integration for Lo Events
...
export async function fetchSeatGeekEvents(
  ...
  try {
    ...
    const normalizedEvents = events.map(normalizeSeatGeekEvent).filter(e => e.lat && e.lng);
    
    console.log(`✅ [${requestId}] SeatGeek: ${normalizedEvents.length} events found`);
    
    return { events: normalizedEvents, source: 'seatgeek' };
  } catch (error) {
    console.error(`❌ [${requestId}] SeatGeek error:`, error);
    throw error;
  }
}
...
export async function fetchPredictHQEvents(
  ...
  try {
    ...
    const normalizedEvents = events.map(normalizePredictHQEvent).filter(e => e.lat && e.lng);
    
    console.log(`✅ [${requestId}] PredictHQ: ${normalizedEvents.length} events found`);
    
    return { events: normalizedEvents, source: 'predicthq' };
  } catch (error) {
    console.error(`❌ [${requestId}] PredictHQ error:`, error);
    throw error;
  }
}
...
function normalizePredictHQEvent(event: any) {
  ...
  return {
    ...
    description: event.description || '',
    event_url: event.url || '',
    image_url: null, // PredictHQ doesn't provide images
...
export async function fetchYelpEvents(
  ...
  // Yelp uses meters for radius (convert miles to meters)
  ...
  try {
    ...
    const normalizedEvents = events.map(normalizeYelpEvent).filter(e => e.lat && e.lng);
    
    console.log(`✅ [${requestId}] Yelp: ${normalizedEvents.length} events found`);
    
    return { events: normalizedEvents, source: 'yelp' };
  } catch (error) {
    console.error(`❌ [${requestId}] Yelp error:`, error);
    throw error;
  }
}
